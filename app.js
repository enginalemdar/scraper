...
async function resolveRedirects(url) {
  try {
    ...
    console.warn("⚠️ Redirect çözümleme hatası:", e.message);
    return url;
  }
}
...
async function scrapePage(url, selector) {
  ...
  try {
    ...
    console.warn("⚠️ Body selector bulunamadı, devam ediliyor...");
  }
...
// Routes
app.get("/", (req, res) => {
  res.send("✅ Scraper API çalışıyor. POST /scrape { url, selector? } gönderin.");
});

app.post("/scrape", async (req, res) => {
  const { url, selector } = req.body;
  if (!url)
    return res.status(400).json({ error: true, message: "url gerekli" });
